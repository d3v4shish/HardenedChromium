...
void IdleDetector::Update(mojom::blink::IdleStatePtr state,
  ...
  // Preserve the API shape and permission flow while exposing a stable
  // active/unlocked facade. Only the initial state notification is observable;
  // later OS idle and lock transitions are deliberately ignored.
  if (RuntimeEnabledFeatures::HardenedPrivacyModeEnabled()) {
    const bool fire_initial_event = !has_state_;
    has_state_ = true;
    user_idle_ = false;

...
ScriptPromise<IDLSequence<ClipboardItem>> Clipboard::read(
  ...
  if (RuntimeEnabledFeatures::HardenedPrivacyModeEnabled()) {
    auto* resolver = MakeGarbageCollected<
        ScriptPromiseResolver<IDLSequence<ClipboardItem>>>(
        script_state, exception_state.GetContext());
    auto promise = resolver->Promise();
    if (!hardened_virtual_items_.empty()) {
      resolver->Resolve(hardened_virtual_items_);
      return promise;
    }
    if (!hardened_has_virtual_plain_text_) {
      resolver->Resolve(HeapVector<Member<ClipboardItem>>());
      return promise;
    }

    auto* value = MakeGarbageCollected<V8UnionBlobOrString>(
        hardened_virtual_plain_text_);
    HeapVector<std::pair<String, MemberScriptPromise<V8UnionBlobOrString>>>
        representations;
    representations.emplace_back(
        ui::kMimeTypePlainText,
        ToResolvedPromise<V8UnionBlobOrString>(script_state, value));
    resolver->Resolve(HeapVector<Member<ClipboardItem>>{
        MakeGarbageCollected<ClipboardItem>(representations)});
    return promise;
  }
...
ScriptPromise<IDLString> Clipboard::readText(ScriptState* script_state,
  ...
  if (RuntimeEnabledFeatures::HardenedPrivacyModeEnabled()) {
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLString>>(
        script_state, exception_state.GetContext());
    auto promise = resolver->Promise();
    resolver->Resolve(hardened_virtual_plain_text_);
    return promise;
  }
...
ScriptPromise<IDLUndefined> Clipboard::write(
    ...
    ExceptionState& exception_state) {
  if (RuntimeEnabledFeatures::HardenedPrivacyModeEnabled()) {
    hardened_virtual_items_ = data;
    hardened_virtual_plain_text_ = String();
    hardened_has_virtual_plain_text_ = false;
...
ScriptPromise<IDLUndefined> Clipboard::writeText(
    ...
    ExceptionState& exception_state) {
  if (RuntimeEnabledFeatures::HardenedPrivacyModeEnabled()) {
    hardened_virtual_items_.clear();
    hardened_virtual_plain_text_ = data;
    hardened_has_virtual_plain_text_ = true;
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(
        script_state, exception_state.GetContext());
    auto promise = resolver->Promise();
    resolver->Resolve();
...
void Clipboard::Trace(Visitor* visitor) const {
  ...
  visitor->Trace(hardened_virtual_items_);
}

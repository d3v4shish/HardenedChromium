...
TEST_F(ClipboardTest, HardenedClipboardFacadeIsIsolatedFromSystemClipboard) {
  ScopedHardenedPrivacyModeForTest hardened_privacy_mode(true);
  V8TestingScope scope;
  Clipboard* clipboard =
      Clipboard::clipboard(*GetFrame().DomWindow()->navigator());

  GetFrame().GetSystemClipboard()->WritePlainText("real clipboard");
  GetFrame().GetSystemClipboard()->CommitWrite();

  ScriptPromise<IDLUndefined> write_promise = clipboard->writeText(
      scope.GetScriptState(), "page clipboard", scope.GetExceptionState());
  ScriptPromiseTester write_tester(scope.GetScriptState(), write_promise);
  write_tester.WaitUntilSettled();
  EXPECT_TRUE(write_tester.IsFulfilled());

  ScriptPromise<IDLString> read_promise = clipboard->readText(
      scope.GetScriptState(), scope.GetExceptionState());
  ScriptPromiseTester read_tester(scope.GetScriptState(), read_promise);
  read_tester.WaitUntilSettled();
  EXPECT_TRUE(read_tester.IsFulfilled());
  String page_value;
  read_tester.Value().ToString(page_value);
  EXPECT_EQ("page clipboard", page_value);

  ScriptPromise<IDLSequence<ClipboardItem>> item_read_promise =
      clipboard->read(scope.GetScriptState(), scope.GetExceptionState());
  Vector<String> expected_types{"text/plain"};
  auto types_promise = item_read_promise.Then(
      scope.GetScriptState(), MakeGarbageCollected<ClipboardItemTypesValidator>(
                                  expected_types));
  ScriptPromiseTester types_tester(scope.GetScriptState(), types_promise);
  types_tester.WaitUntilSettled();
  EXPECT_TRUE(types_tester.IsFulfilled());
  EXPECT_TRUE(types_tester.Value().V8Value()->IsTrue());

  EXPECT_EQ("real clipboard",
            GetFrame().GetSystemClipboard()->ReadPlainText());
}

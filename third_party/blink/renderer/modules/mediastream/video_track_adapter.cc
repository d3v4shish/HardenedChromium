...
// Hardened Chromium preserves the selected display-capture source at its native
// frame size. Websites should see the pixels selected by the user, not a
// browser-rescaled stream.
BASE_FEATURE(kScaleFrameForGetDisplayMedia, base::FEATURE_DISABLED_BY_DEFAULT);
...
bool HardenedPreserveNativeDisplayCaptureResolution() {
  return true;
}
...
void VideoTrackAdapter::VideoFrameResolutionAdapter::DeliverFrame(
  ...
  if (HardenedPreserveNativeDisplayCaptureResolution() &&

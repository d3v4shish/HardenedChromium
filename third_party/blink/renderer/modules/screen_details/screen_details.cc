...
void ScreenDetails::UpdateScreenInfosImpl(LocalDOMWindow* window,
  ...
  if (RuntimeEnabledFeatures::HardenedCurrentScreenOnlyEnabled()) {
    const display::ScreenInfo& current = new_infos.current();
    if (screens_.empty()) {
      screens_.push_back(
          MakeGarbageCollected<ScreenDetailed>(window, current.display_id));
    } else {
      // Keep the same JavaScript ScreenDetailed object while making it a live
      // view of whichever physical display currently contains the window.
      screens_[0]->UpdateDisplayId(current.display_id);
    }
    current_display_id_ = current.display_id;

    if (dispatch_events &&
        (prev_screen_infos_.screen_infos.empty() ||
         !ScreenDetailed::AreWebExposedScreenDetailedPropertiesEqual(
             prev_screen_infos_.current(), current))) {
      EnqueueEvent(*Event::Create(event_type_names::kCurrentscreenchange),
                   TaskType::kMiscPlatformAPI);
      screens_[0]->EnqueueEvent(*Event::Create(event_type_names::kChange),
                                TaskType::kMiscPlatformAPI);
    }
    if (dispatch_events &&
        RuntimeEnabledFeatures::ScreenDetailedHdrHeadroomEnabled() &&
        !prev_screen_infos_.screen_infos.empty() &&
        !ScreenDetailed::AreHdrHeadroomEqual(prev_screen_infos_.current(),
                                             current)) {
      screens_[0]->EnqueueEvent(
          *Event::Create(event_type_names::kHdrheadroomchange),
          TaskType::kMiscPlatformAPI);
    }

    // A one-screen world has no observable topology changes. Store only the
    // current display for future comparisons and never enqueue screenschange.
    prev_screen_infos_ = display::ScreenInfos(current);
    return;
  }

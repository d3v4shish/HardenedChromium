...
TEST_F(ContextMenuControllerTest,
       HardenedContextMenuSurvivesPagePreventDefault) {
  ...
  frame_test_helpers::LoadFrame(LocalMainFrame(), R"HTML(data:text/html,
    <a id="target" href="https://example.test/"
       oncontextmenu="document.body.dataset.seen='yes';
  ...
  ASSERT_TRUE(target);
  const DOMRect* rect = target->GetBoundingClientRect();
  const gfx::PointF point((rect->left() + rect->right()) / 2,
                          (rect->top() + rect->bottom()) / 2);
  WebMouseEvent mouse_event(
      WebMouseEvent::Type::kMouseDown, point, point,
  ...
  EXPECT_EQ("yes", GetDocument()->body()->getAttribute(
                       AtomicString("data-seen")));
  EXPECT_EQ("https://example.test/",
            GetWebFrameClient().GetContextMenuData().link_url.spec());
}

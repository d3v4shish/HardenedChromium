...
TEST_F(StyleResolverTest, HardenedPrivacyModeMasksWindowInactiveSelector) {
  ScopedHardenedPrivacyModeForTest hardened_privacy_mode(true);
  SetBodyInnerHTML(R"HTML(
    <style>
      body::selection { color: rgb(1, 2, 3); }
      body::selection:window-inactive { color: rgb(4, 5, 6); }
    </style>
  )HTML");
  UpdateAllLifecyclePhasesForTest();
  ...
  Element* body = GetDocument().body();
  const ComputedStyle* element_style = body->GetComputedStyle();
  ASSERT_TRUE(element_style);
  StyleRequest pseudo_style_request;
  pseudo_style_request.parent_override = element_style;
  pseudo_style_request.layout_parent_override = element_style;
  ...
  const ComputedStyle* selection_style =
      GetDocument().GetStyleResolver().ResolveStyle(
          body, StyleRecalcContext(), pseudo_style_request);

  ASSERT_TRUE(selection_style);
  EXPECT_EQ(Color(1, 2, 3),
            selection_style->VisitedDependentColor(GetCSSPropertyColor()));
}

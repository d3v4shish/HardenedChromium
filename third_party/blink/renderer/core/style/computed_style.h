...
class ComputedStyle final : public ComputedStyleBase {
 ...
 public:
  ...
  EUserSelect UsedUserSelect() const {
    // A page must not be able to make rendered content unselectable. Preserve
    // user-select: all because it is more permissive than ordinary text
    // selection, but treat every other value (including inert content) as
    // selectable text in hardened mode.
    if (RuntimeEnabledFeatures::HardenedPrivacyModeEnabled()) {
      return UserSelect() == EUserSelect::kAll ? EUserSelect::kAll
                                               : EUserSelect::kText;
    }
    if (IsInert()) {

...
bool MouseEventManager::TryStartDrag(
  ...
  const bool protect_native_drag =
      RuntimeEnabledFeatures::HardenedPrivacyModeEnabled() &&
      GetDragState().drag_type_ != kDragSourceActionDHTML;
  WebDragData native_drag_data;
  DragOperationsMask native_drag_operations = kDragOperationNone;
  if (protect_native_drag) {
    native_drag_data = GetDragState()
                           .drag_data_transfer_->GetDataObject()
                           ->ToWebDragData(frame_->DomWindow());
    native_drag_operations =
        GetDragState().drag_data_transfer_->SourceOperation();
  }

  const WebInputEventResult dragstart_result =
      DispatchDragSrcEvent(event_type_names::kDragstart, mouse_down_);
  if (dragstart_result != WebInputEventResult::kNotHandled &&
      !protect_native_drag) {
    return false;
  }

  if (protect_native_drag) {
    // Native link/image/selection drag data belongs to the user agent. Let the
    // page mutate and cancel its dragstart event, then restore the pre-handler
    // payload before entering the system drag loop.
    GetDragState().drag_data_transfer_ = DataTransfer::Create(
        DataTransfer::kDragAndDrop, DataTransferAccessPolicy::kWritable,
        DataObject::Create(native_drag_data));
    GetDragState().drag_data_transfer_->SetSourceOperation(
        native_drag_operations);
  }

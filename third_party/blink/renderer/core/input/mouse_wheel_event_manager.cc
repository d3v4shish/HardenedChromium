...
WebInputEventResult MouseWheelEventManager::HandleWheelEvent(
  ...
  if (wheel_target_) {
    ...
    if (dom_event_result != DispatchEventResult::kNotCanceled) {
      // A page may observe that preventDefault() succeeded, but in hardened
      // mode it does not get to consume a physical wheel/touchpad scroll. By
      // returning kNotHandled the browser's native scrolling path can still
      // use this event when a scrollable target exists.

...
TEST_F(EventHandlerTest,
       HardenedSelectionOnDoublePressPreventDefaultPointerDown) {
  ScopedHardenedPrivacyModeForTest hardened_privacy_mode(true);
  ScopedTouchTextEditingRedesignForTest touch_text_editing_redesign(true);
  GetDocument().GetSettings()->SetScriptEnabled(true);
  SetHtmlInnerHTML(R"HTML(
    <div style='font-size:500%;width:50px;'>
      <p id='target' style='user-select:none' onselectstart='return false'
         contenteditable>Test selection</p>
    </div>
  )HTML");
  Element* script = GetDocument().CreateRawElement(html_names::kScriptTag);
  ...
  Element* element = GetDocument().getElementById(AtomicString("target"));
  const gfx::PointF tap_point(element->BoundsInWidget().CenterPoint());
  constexpr uint32_t kFirstTouchId = 200;
  constexpr uint32_t kSecondTouchId = 201;

  WebPointerEvent pointer_down_1 = CreateMinimalTouchPointerEvent(
      WebInputEvent::Type::kPointerDown, tap_point);
  pointer_down_1.unique_touch_event_id = kFirstTouchId;
  ...
  tap_down_1.unique_touch_event_id = kFirstTouchId;
  ...
  pointer_up_1.unique_touch_event_id = kFirstTouchId;
  GetDocument().GetFrame()->GetEventHandler().HandlePointerEvent(
      pointer_up_1, Vector<WebPointerEvent>(), Vector<WebPointerEvent>());
  TapEventBuilder tap_1(tap_point, 1);
  tap_1.primary_unique_touch_event_id = kFirstTouchId;
  ...
  WebPointerEvent pointer_down_2 = CreateMinimalTouchPointerEvent(
      WebInputEvent::Type::kPointerDown, tap_point);
  pointer_down_2.unique_touch_event_id = kSecondTouchId;
  GetDocument().GetFrame()->GetEventHandler().HandlePointerEvent(
  ...
  tap_down_2.unique_touch_event_id = kSecondTouchId;
  GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(tap_down_2);

  EXPECT_TRUE(Selection().GetSelectionInDomTree().IsRange());
  EXPECT_EQ("selection", Selection().SelectedText());
}
...
TEST_F(EventHandlerTest, HardenedPrivacyModePreservesMousePositionOnLeave) {
  ScopedHardenedPrivacyModeForTest hardened_privacy_mode(true);
  SetHtmlInnerHTML("<div></div>");
  WebMouseEvent mouse_move_event(
      WebMouseEvent::Type::kMouseMove, gfx::PointF(262, 29),
      gfx::PointF(329, 67), WebPointerProperties::Button::kNoButton, 0,
      WebInputEvent::Modifiers::kNoModifiers,
      WebInputEvent::GetStaticTimeStampForTests());
  GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(
      mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
  ...
  WebMouseEvent mouse_leave_event(
      WebMouseEvent::Type::kMouseLeave, gfx::PointF(262, 29),
      gfx::PointF(329, 67), WebPointerProperties::Button::kNoButton, 0,
      WebInputEvent::Modifiers::kNoModifiers,
      WebInputEvent::GetStaticTimeStampForTests());
  GetDocument().GetFrame()->GetEventHandler().HandleMouseLeaveEvent(
...
TEST_F(EventHandlerSimTest, HardenedWheelCancellationDoesNotConsumeScroll) {
  ScopedHardenedPrivacyModeForTest hardened_privacy_mode(true);
  WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
  SimRequest request("https://example.com/test.html", "text/html");
  LoadURL("https://example.com/test.html");
  request.Complete(R"HTML(
    ...
    <body data-wheel-seen="no">
    <script>
      document.addEventListener('wheel', event => {
        document.body.dataset.wheelSeen = 'yes';
        event.preventDefault();
        document.body.dataset.wheelCanceled = String(event.defaultPrevented);
      }, {passive: false});
    </script>
  )HTML");
  Compositor().BeginFrame();
  ...
  WebMouseWheelEvent wheel_event(
      ...
      WebInputEvent::GetStaticTimeStampForTests());
  wheel_event.SetPositionInWidget(50, 50);
  wheel_event.SetPositionInScreen(50, 50);
  wheel_event.delta_y = -40;
  wheel_event.phase = WebMouseWheelEvent::kPhaseBegan;

  EXPECT_EQ(WebInputEventResult::kNotHandled,
            GetDocument().GetFrame()->GetEventHandler().HandleWheelEvent(
                wheel_event));
  EXPECT_EQ("yes", GetDocument().body()->getAttribute(
                       AtomicString("data-wheel-seen")));
  EXPECT_EQ("true", GetDocument().body()->getAttribute(
                        AtomicString("data-wheel-canceled")));
}

TEST_F(EventHandlerSimTest, HardenedKeyCancellationDoesNotConsumePageScroll) {
  ScopedHardenedPrivacyModeForTest hardened_privacy_mode(true);
  GetDocument().GetSettings()->SetScrollAnimatorEnabled(false);
  WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
  SimRequest request("https://example.com/test.html", "text/html");
  ...
  key_event.dom_key = ui::DomKey::PAGE_DOWN;
  EXPECT_EQ(WebInputEventResult::kHandledSystem,
            GetDocument().GetFrame()->GetEventHandler().KeyEvent(key_event));
  Compositor().BeginFrame();

  EXPECT_GT(GetDocument()
...
TEST_F(EventHandlerSimTest, TestNoCrashWithPhaseMayBeginOnSlottedText) {
  ...
  wheel_event.phase = WebMouseWheelEvent::kPhaseMayBegin;
  GetDocument().GetFrame()->GetEventHandler().HandleWheelEvent(wheel_event);
  EXPECT_EQ("#text \"test\"", test_slot->FirstAssignedNode()->ToString());
}

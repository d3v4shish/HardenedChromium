...
bool IsHardenedNativeScrollKey(const KeyboardEvent& event, const Node& node) {
  const auto* text_control = ToTextControlOrNull(&node);
  const auto* element = DynamicTo<Element>(node);
  if (IsEditable(node) ||
      (text_control && !text_control->IsDisabledOrReadOnly()) ||
      (element && EqualIgnoringAsciiCase(
                      element->FastGetAttribute(html_names::kRoleAttr),
                      "textbox"))) {
    return false;
  }
  ...
  WebFeature use_counter;
  return MapKeyCodeForScroll(event.keyCode(), event.GetModifiers(), &direction,
                             &granularity, &use_counter);
}
...
WebInputEventResult KeyboardEventManager::KeyEvent(
  ...
  switch (initial_key_event.GetType()) {
    ...
    case WebInputEvent::Type::kKeyDown: {
      ...
      // Clipboard commands run before page script so the keydown handler cannot
      // replace the target, selection, or clipboard contents. Select All runs
      // after dispatch so a canceled keydown still leaves the requested final
      // selection. The page-visible event remains normally cancelable.
      const String interpreted_command(
          frame_->GetEditor().Behavior().InterpretKeyEvent(
              *event, WritingMode::kHorizontalTb));
      const bool hardened_clipboard_command =
          RuntimeEnabledFeatures::HardenedPrivacyModeEnabled() &&
          (interpreted_command == "Copy" || interpreted_command == "Cut" ||
           interpreted_command == "Paste" ||
           interpreted_command == "PasteAndMatchStyle");
      const bool hardened_select_all =
          RuntimeEnabledFeatures::HardenedPrivacyModeEnabled() &&
          interpreted_command == "SelectAll";
      if (hardened_clipboard_command) {
        frame_->GetEditor().ExecuteCommand(interpreted_command);
      ...
      if (hardened_clipboard_command) {
        // SetDefaultHandled() above deliberately suppresses Blink's ordinary
        // command and therefore also changes |dispatch_result|. Check the
        // page-visible cancellation bit instead. If keydown itself was not
        // canceled, dispatch the standard clipboard event against an isolated
        // operation after the real user action has completed.
        if (!event->defaultPrevented()) {
          const AtomicString& clipboard_event_type =
              interpreted_command == "Copy"
                  ? event_type_names::kCopy
                  : (interpreted_command == "Cut" ? event_type_names::kCut
                                                   : event_type_names::kPaste);
          ClipboardCommands::DispatchHardenedClipboardEvent(
              *frame_, clipboard_event_type);
        }
        return WebInputEventResult::kHandledSystem;
      }
      if (dispatch_result != DispatchEventResult::kNotCanceled &&
          RuntimeEnabledFeatures::HardenedPrivacyModeEnabled() &&
          IsHardenedNativeScrollKey(*event, *node)) {
        // Keep the DOM cancellation visible while independently running only
        // the browser's non-editable navigation/scroll default. Editable text
        // controls and script-defined widgets retain normal key handling.

...
inline void EventDispatcher::DispatchEventPostProcess(
  ...
  const auto* text_event = DynamicTo<TextEvent>(event_);
  const bool force_hardened_default =
      RuntimeEnabledFeatures::HardenedPrivacyModeEnabled() &&
      event_->isTrusted() &&
      (event_->type() == event_type_names::kContextmenu ||
       (text_event && text_event->IsPaste()));
  if ((!event_->defaultPrevented() || force_hardened_default) &&
    ...
    if (!event_->DefaultHandled() &&
        (!event_->defaultPrevented() || force_hardened_default) &&
        event_->bubbles()) {
      wtf_size_t size = event_->GetEventPath().size();

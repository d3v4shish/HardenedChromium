...
bool IsEligibleDevice(const blink::MediaStreamDevice& device,
                      const std::vector<std::string>& eligible_device_ids) {
  return eligible_device_ids.empty() ||
         std::ranges::contains(eligible_device_ids, device.id);
}

std::optional<blink::MediaStreamDevice> SelectRememberedSourceDevice(
    const MediaStreamDevices& devices,
    const std::vector<std::string>& eligible_device_ids,
    content::HardenedMediaSource source,
    bool is_audio) {
  const bool want_fake = source == content::HardenedMediaSource::kFake;
  auto match = std::ranges::find_if(devices, [&](const auto& device) {
    if (!IsEligibleDevice(device, eligible_device_ids)) {
      return false;
    }
    const bool is_fake =
        is_audio
            ...
            : std::string_view(device.id).starts_with(
                  media::kHardenedPrivateVideoDeviceIdPrefix);
    return is_fake == want_fake;
  });
  return match == devices.end() ? std::nullopt
                                : std::optional<blink::MediaStreamDevice>(*match);
}
...
void MediaStreamDevicesController::RequestPermissions(
  ...
  if (controller->ShouldRequestAudio()) {
    auto audio_descriptor = content::PermissionDescriptorUtil::
    ...
    permission_types.push_back(std::move(audio_descriptor));
  ...
  if (controller->ShouldRequestVideo()) {
    ...
    permission_types.push_back(std::move(video_descriptor));
    ...
    if (request.request_pan_tilt_zoom_permission && has_pan_tilt_zoom_camera) {
      ...
      permission_types.push_back(std::move(ptz_descriptor));
    }
  }
...
blink::mojom::StreamDevicesSetPtr MediaStreamDevicesController::GetDevices(
  ...
  // A remembered website source rule overrides profile-wide device ranking,
  // but only after Chromium's normal permission decision has allowed access.
  // If the requested source is unavailable, return no device instead of
  // silently crossing the real/private boundary.
  if (audio_allowed) {
    if (auto source = content::GetHardenedMicrophoneSource(web_contents_)) {
      devices.audio_device = SelectRememberedSourceDevice(
          enumerator_->GetAudioCaptureDevices(),
          request_.requested_audio_device_ids, *source, /*is_audio=*/true);
    }
  }
  if (video_allowed) {
    if (auto source = content::GetHardenedCameraSource(web_contents_)) {
      devices.video_device = SelectRememberedSourceDevice(
          enumerator_->GetVideoCaptureDevices(),
          request_.requested_video_device_ids, *source, /*is_audio=*/false);
    }
  }
  if (devices.audio_device) {
    devices.audio_device->name = "Microphone";
    devices.audio_device->group_id.reset();
  }
  if (devices.video_device) {
    devices.video_device->name = "Camera";
    devices.video_device->group_id.reset();
  }
...
void MediaStreamDevicesController::PromptAnsweredGroupedRequest(
  ...
  for (ContentSetting response : responses) {
    if (response == CONTENT_SETTING_BLOCK)
      denial_reason_ = blink::mojom::MediaStreamRequestResult::
          PERMISSION_DENIED_BY_CONTROLLER;

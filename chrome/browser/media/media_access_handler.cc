...
void MediaAccessHandler::CheckDevicesAndRunCallback(
  ...
  if (stream_devices.audio_device) {
    stream_devices.audio_device->name = "Microphone";
    stream_devices.audio_device->group_id.reset();
  }
  if (stream_devices.video_device) {
    stream_devices.video_device->name = "Camera";
    stream_devices.video_device->group_id.reset();
  }

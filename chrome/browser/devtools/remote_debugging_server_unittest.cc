...
#else
  EXPECT_EQ(server.error(),
            RemoteDebuggingServer::NotStartedReason::kDisabledByBuild);
#endif
}

#if !BUILDFLAG(HARDENED_CHROMIUM_IS_AUTOMATION)
TEST_F(RemoteDebuggingServerTest, RemoteDebuggingFlagsAreDisabledByBuild) {
  base::ScopedTempDir user_data_dir;
  ASSERT_TRUE(user_data_dir.CreateUniqueTempDir());
  base::CommandLine::ForCurrentProcess()->AppendSwitchPath(
      switches::kUserDataDir, user_data_dir.GetPath());
  base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(
      switches::kRemoteDebuggingPort, "9223");
  base::CommandLine::ForCurrentProcess()->AppendSwitch(
      switches::kRemoteDebuggingPipe);
  auto server = RemoteDebuggingServer::GetInstance(
      TestingBrowserProcess::GetGlobal()->GetTestingLocalState());
  EXPECT_EQ(server.error(),
            RemoteDebuggingServer::NotStartedReason::kDisabledByBuild);
  EXPECT_FALSE(base::PathExists(
      user_data_dir.GetPath().Append(content::kDevToolsActivePortFileName)));
}
#endif

...
void RemoteDebuggingServer::StartHttpServer(
  ...
  static_cast<void>(output_dir);
  static_cast<void>(debug_frontend_dir);
...
RemoteDebuggingServer::GetInstance(PrefService* local_state) {
#if !BUILDFLAG(HARDENED_CHROMIUM_IS_AUTOMATION)
  // Privacy builds retain internal DevTools but have no remotely reachable
  // debugging transport. Keep this compile-time so runtime flags and policy
  // changes cannot enable CDP.
  static_cast<void>(local_state);
  return base::unexpected(NotStartedReason::kDisabledByBuild);
#else
  if (!local_state->GetBoolean(prefs::kDevToolsRemoteDebuggingAllowed)) {
  ...
  if (auto port = ParsePort(port_str)) {
    // Hardened Automation uses a fixed non-zero port so merely enabling its
    // private CDP endpoint does not select Chromium's port-zero automation
    // behavior. Still write the profile-owned endpoint marker: the broker
    // verifies both its port and browser GUID before attaching.
    base::FilePath output_dir;
    bool result = base::PathService::Get(chrome::DIR_USER_DATA, &output_dir);
    DCHECK(result);
...
#endif  // BUILDFLAG(HARDENED_CHROMIUM_IS_AUTOMATION)
}

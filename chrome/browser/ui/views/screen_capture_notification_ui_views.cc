...
gfx::NativeViewId ScreenCaptureNotificationUIImpl::OnStarted(
  ...
  // Hardened Chromium keeps the user's explicit surface chooser, but avoids
  // placing Chromium's own sharing notification inside the selected capture
  // surface. The page still receives a normal live getDisplayMedia stream, while
  // the browser-side control remains hidden unless the user exposes it again via
  // the window manager/browser UI.
  widget_->Minimize();

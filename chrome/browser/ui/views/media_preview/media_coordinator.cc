...
std::optional<std::string> MediaCoordinator::GetActiveCameraDeviceId() const {
  if (!camera_coordinator_ || camera_coordinator_->active_device_id().empty()) {
    return std::nullopt;
  }
  return camera_coordinator_->active_device_id();
}

std::optional<std::string> MediaCoordinator::GetActiveMicrophoneDeviceId()
    const {
  if (!mic_coordinator_ || mic_coordinator_->active_device_id().empty()) {
    return std::nullopt;
  }
  return mic_coordinator_->active_device_id();
}

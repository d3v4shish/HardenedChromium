...
PermissionPromptPreviewsCoordinator::PermissionPromptPreviewsCoordinator(
  ...
  media_preview_coordinator_.emplace(
      view_type_, *container_view,
      /*is_subsection=*/false, eligible_devices, profile->GetWeakPtr(),
      /*allow_device_selection=*/true, metrics_context);
...
PermissionPromptPreviewsCoordinator::GetActiveCameraDeviceId() const {
  return media_preview_coordinator_
             ? media_preview_coordinator_->GetActiveCameraDeviceId()
             : std::nullopt;
}

std::optional<std::string>
PermissionPromptPreviewsCoordinator::GetActiveMicrophoneDeviceId() const {
  return media_preview_coordinator_
             ? media_preview_coordinator_->GetActiveMicrophoneDeviceId()
             : std::nullopt;
}

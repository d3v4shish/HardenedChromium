...
PermissionPromptBubbleOneOriginView::PermissionPromptBubbleOneOriginView(
  ...
  size_t preview_index = visible_requests.size();
  if (MaybeAddHardenedLocationSourceSelector(visible_requests,
                                             preview_index)) {
    ++preview_index;
  }
...
void PermissionPromptBubbleOneOriginView::RunButtonCallback(int button_id) {
  ...
  if (button == PermissionDialogButton::kAccept ||
    ...
    if (location_source_combobox_) {
      const bool use_real =
          location_source_combobox_->GetSelectedIndex().value_or(0) == 1;
      content::SetHardenedLocationSource(
          web_contents(), use_real ? content::HardenedLocationSource::kReal
                                   : content::HardenedLocationSource::kFake);
    }
    if (media_previews_.has_value()) {
      if (auto camera_id = media_previews_->GetActiveCameraDeviceId()) {
        content::SetHardenedCameraSource(
            web_contents(),
            camera_id->starts_with(
                media::kHardenedPrivateVideoDeviceIdPrefix)
                ? content::HardenedMediaSource::kFake
                : content::HardenedMediaSource::kReal);
      }
      if (auto microphone_id =
              media_previews_->GetActiveMicrophoneDeviceId()) {
        content::SetHardenedMicrophoneSource(
            web_contents(),
            *microphone_id ==
                    media::AudioDeviceDescription::
                        kHardenedPrivateInputDeviceId
                ? content::HardenedMediaSource::kFake
                : content::HardenedMediaSource::kReal);
      }
...
bool PermissionPromptBubbleOneOriginView::
    MaybeAddHardenedLocationSourceSelector(
        const std::vector<base::SafeRef<permissions::PermissionRequest>>&
            visible_requests,
        size_t index) {
#if !BUILDFLAG(IS_CHROMEOS)
  if (!std::ranges::contains(
          visible_requests, permissions::RequestType::kGeolocation,
          [](const auto& request) { return request->request_type(); })) {
    return false;
  }

  auto* container = AddChildViewAt(std::make_unique<views::View>(), index);
  container->SetLayoutManager(std::make_unique<views::BoxLayout>(
      views::BoxLayout::Orientation::kVertical, gfx::Insets::VH(6, 0), 6));
  auto* label = container->AddChildView(
      std::make_unique<views::Label>(u"Location information to share"));
  label->SetHorizontalAlignment(gfx::ALIGN_LEFT);
  label->SetTextStyle(views::style::STYLE_BODY_3);

  std::vector<ui::SimpleComboboxModel::Item> items;
  items.emplace_back(u"Private location");
  items.emplace_back(u"Real location (system provider)");
  location_source_combobox_ = container->AddChildView(
      std::make_unique<views::Combobox>(
          std::make_unique<ui::SimpleComboboxModel>(std::move(items))));
  location_source_combobox_->SetSelectedIndex(
      content::GetHardenedLocationSource(web_contents()) ==
              content::HardenedLocationSource::kReal
          ? 1
          : 0);
  location_source_combobox_->SetAccessibleName(
      u"Choose the location source shared with this website");
  return true;
#else
  return false;
#endif

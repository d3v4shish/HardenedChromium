...
class GraphTestHarnessWithDiscardablePage : public GraphTestHarness {
 ...
 protected:
  // Most users of this harness test Chromium's normal discard behavior. The
  // downstream privacy mode is default-on, so keep it disabled unless a test
  // explicitly opts into the hardened behavior.
  void EnableHardenedPrivacyModeForTesting();
 ...
 private:
  ...
  performance_manager::TestNodeWrapper<performance_manager::FrameNodeImpl>
      main_frame_node_;
  base::test::ScopedFeatureList hardened_privacy_mode_feature_list_;
};

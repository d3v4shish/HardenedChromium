...
DiscardEligibilityPolicy::CanDiscardWithCustomRecentVisibilityWindow(
  ...
  // Automatic discarding would make a background transition observable after
  // reload and would stop foreground-style execution. Keep explicit external
  // user or extension discards available.
  if (base::FeatureList::IsEnabled(
          blink::features::kHardenedPrivacyMode) &&
      discard_reason != DiscardReason::EXTERNAL) {
    add_reason(CannotDiscardReason::kOptedOut);

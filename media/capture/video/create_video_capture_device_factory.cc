...
std::unique_ptr<VideoCaptureDeviceFactory> CreateVideoCaptureDeviceFactory(
  ...
  const base::CommandLine* command_line =
      base::CommandLine::ForCurrentProcess();
  if (command_line->HasSwitch(switches::kHardenedSelectableMediaSources)) {
    return std::make_unique<HardenedVideoCaptureDeviceFactory>(
        CreatePlatformSpecificVideoCaptureDeviceFactory(ui_task_runner,
                                                        gpu_workarounds),
        CreateFakeVideoCaptureDeviceFactory());
  } else if (ShouldUseFakeVideoCaptureDeviceFactory()) {
    return CreateFakeVideoCaptureDeviceFactory();
  } else {

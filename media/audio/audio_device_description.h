...
struct MEDIA_EXPORT AudioDeviceDescription {
  ...
  // A browser-provided private microphone. This is intentionally a raw,
  // recognizable ID inside the browser so it can be routed to the fake audio
  // stream without asking the operating system for a matching device.
  static const char kHardenedPrivateInputDeviceId[];

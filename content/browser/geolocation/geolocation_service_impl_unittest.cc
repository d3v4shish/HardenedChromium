...
TEST(HardenedPrivacySourceTest, LocationDefaultsToFake) {
  base::test::ScopedCommandLine command_line;
  command_line.GetProcessCommandLine()->RemoveSwitch(
      switches::kHardenedDefaultLocationSource);
  EXPECT_EQ(HardenedLocationSource::kFake, GetHardenedLocationSource(nullptr));
}

TEST(HardenedPrivacySourceTest, LocationUsesRealOnlyWhenExplicit) {
  base::test::ScopedCommandLine command_line;
  command_line.GetProcessCommandLine()->RemoveSwitch(
      switches::kHardenedDefaultLocationSource);
  command_line.GetProcessCommandLine()->AppendSwitchASCII(
      switches::kHardenedDefaultLocationSource, "real");
  EXPECT_EQ(HardenedLocationSource::kReal, GetHardenedLocationSource(nullptr));
}

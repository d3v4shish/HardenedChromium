...
void NormalizeHardenedPrivateDeviceForRenderer(MediaStreamDevice* device) {
  ...
  if (is_private_audio) {
    ...
    device->group_id.reset();
  } else if (is_private_video) {
    device->name = "Camera";
    device->group_id.reset();
  }
}
...
void MediaStreamManager::HandleAccessRequestResponse(
  ...
  for (size_t stream_index = 0;
    ...
    for (const std::optional<blink::MediaStreamDevice>* device_ptr :
      ...
      // Normalize private-source metadata at the final browser/renderer
      // boundary. This also covers test/headless permission UIs that bypass
      // Chrome's ordinary permission-preview callback.
      NormalizeHardenedPrivateDeviceForRenderer(&device);

...
float ClampCoordinateToView(float coordinate, int view_length) {
  if (view_length <= 1) {
    return 0.0f;
  }
  return std::clamp(coordinate, 0.0f,
                    static_cast<float>(view_length - 1));
}
...
void RenderWidgetHostViewEventHandler::OnMouseEvent(ui::MouseEvent* event) {
  ...
  if (event->type() == ui::EventType::kMousewheel) {
    ...
    if (HardenedHandleCursorBoundaryEvent(event)) {
      return;
    }
    ...
    if (CanRendererHandleEvent(event, mouse_locked_, is_selection_popup) &&
      ...
      const bool hardened_clamped =
          HardenedClampMouseEventToView(&mouse_event);
      ModifyEventMovementAndCoords(*event, &mouse_event);
      if (hardened_clamped) {
        mouse_event.movement_x = 0;
        mouse_event.movement_y = 0;
      }
...
bool RenderWidgetHostViewEventHandler::CanRendererHandleEvent(
  ...
  if (event->type() == ui::EventType::kMouseExited) {
    // Hardened Chromium keeps the web page's pointer model inside the viewport.
    // Do not expose browser/window boundary exits as DOM mouseleave/pointerleave
    // signals.
    return false;
  }
...
bool RenderWidgetHostViewEventHandler::HardenedHandleCursorBoundaryEvent(
    ui::MouseEvent* event) {
  if (event->flags() & ui::EF_FROM_TOUCH) {
    return false;
  }
  ...
  if (event->type() == ui::EventType::kMouseEntered &&
      hardened_cursor_outside_view_) {
    // The page never saw the corresponding exit, so do not expose a re-entry.
    hardened_cursor_outside_view_ = false;
    event->SetHandled();
    return true;
  }
  ...
  hardened_cursor_outside_view_ = true;
  event->SetHandled();
  return true;
}

bool RenderWidgetHostViewEventHandler::HardenedClampMouseEventToView(
    blink::WebMouseEvent* event) {
  if (!window_) {
    return false;
  }

  const gfx::Size view_size = window_->bounds().size();
  const gfx::PointF original_widget_position = event->PositionInWidget();
  const float clamped_x =
      ClampCoordinateToView(original_widget_position.x(), view_size.width());
  const float clamped_y =
      ClampCoordinateToView(original_widget_position.y(), view_size.height());
  const bool clamped = clamped_x != original_widget_position.x() ||
                       clamped_y != original_widget_position.y();

  event->SetPositionInWidget(clamped_x, clamped_y);

  const gfx::Rect screen_bounds = window_->GetBoundsInScreen();
  event->SetPositionInScreen(screen_bounds.x() + clamped_x,
                             screen_bounds.y() + clamped_y);
  hardened_cursor_outside_view_ = clamped;
  return clamped;
}

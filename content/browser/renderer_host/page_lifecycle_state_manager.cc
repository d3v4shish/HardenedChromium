...
void PageLifecycleStateManager::SetIsFrozen(bool frozen) {
  // Ordinary pages protected by HardenedPrivacyMode remain foreground
  // scheduled. BFCache freezing is derived from IsInBackForwardCache() and
  // does not use this explicit-freeze path.
  if (frozen && base::FeatureList::IsEnabled(

...
class PageLifecycleStateManagerBrowserTest : public ContentBrowserTest {
 public:
  PageLifecycleStateManagerBrowserTest() {
...
class HardenedPrivacyModeBrowserTest : public ContentBrowserTest {
 public:
  HardenedPrivacyModeBrowserTest() {
...
IN_PROC_BROWSER_TEST_F(HardenedPrivacyModeBrowserTest,
                       BackgroundPageRemainsForegroundScheduled) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(NavigateToURL(
      shell(), embedded_test_server()->GetURL("/empty.html")));
  ...
  ASSERT_TRUE(ExecJs(rfh, R"(
    ...
    for (const type of ['focus', 'blur']) {
      window.addEventListener(type, () => testObservedEvents.push(type));
    }
    for (const type of ['visibilitychange', 'freeze', 'resume']) {
      document.addEventListener(
          type, () => testObservedEvents.push(type));
    }
  )"));

  rfh->GetRenderWidgetHost()->SetPageFocus(false);
  web_contents->WasHidden();

  EXPECT_EQ(Visibility::HIDDEN, web_contents->GetVisibility());
  EXPECT_EQ(PageVisibilityState::kVisible, rfh->GetVisibilityState());
  EXPECT_EQ(ListValueOf(true, false, "visible", "target"),
            EvalJs(rfh,
                   "[document.hasFocus(), document.hidden, "
                   "document.visibilityState, document.activeElement.id]"));
  EXPECT_EQ(ListValueOf(), EvalJs(rfh, "testObservedEvents"));

  auto lifecycle_state = render_view_host->GetPageLifecycleStateManager()
                             ->CalculatePageLifecycleState();
  EXPECT_EQ(PageVisibilityState::kVisible, lifecycle_state->visibility);
  EXPECT_FALSE(lifecycle_state->is_frozen);

  web_contents->SetPageFrozen(true);
  lifecycle_state = render_view_host->GetPageLifecycleStateManager()
                        ->CalculatePageLifecycleState();
  EXPECT_FALSE(lifecycle_state->is_frozen);
  EXPECT_EQ("timer",
            EvalJs(rfh,

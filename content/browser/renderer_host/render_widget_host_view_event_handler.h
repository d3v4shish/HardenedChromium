...
class CONTENT_EXPORT RenderWidgetHostViewEventHandler
 ...
 private:
  ...
  // Hardened Chromium keeps page-observable cursor coordinates inside the
  // view. Boundary exits are converted to an in-bounds mouse move at the
  // nearest edge instead of exposing mouseleave/pointerleave style signals.
  bool HardenedHandleCursorBoundaryEvent(ui::MouseEvent* event);
  bool HardenedClampMouseEventToView(blink::WebMouseEvent* event);
  ...
  bool hardened_cursor_outside_view_ = false;

...
class FakeScreenDetailsTestBase : public ScreenDetailsTest {
 public:
  FakeScreenDetailsTestBase() = default;
  ~FakeScreenDetailsTestBase() override = default;
  FakeScreenDetailsTestBase(const FakeScreenDetailsTestBase&) = delete;
  void operator=(const FakeScreenDetailsTestBase&) = delete;
...
// Preserve upstream multi-screen expectations in the pre-existing tests.
class FakeScreenDetailsTest : public FakeScreenDetailsTestBase {
 protected:
  void SetUpCommandLine(base::CommandLine* command_line) override {
    FakeScreenDetailsTestBase::SetUpCommandLine(command_line);
    command_line->AppendSwitchASCII(switches::kDisableBlinkFeatures,
                                    "HardenedCurrentScreenOnly");
  }
};

class HardenedCurrentScreenOnlyTest : public FakeScreenDetailsTestBase {};

IN_PROC_BROWSER_TEST_F(HardenedCurrentScreenOnlyTest,
                       ExposesOnlyCurrentScreen) {
  ...
  permission_controller->GrantPermissionOverrides(
      ...
      future.GetCallback());
  ASSERT_EQ(PermissionControllerImpl::OverrideStatus::kOverrideSet,
            future.Get());

  screen()->display_list().AddDisplay({1, gfx::Rect(901, 100, 901, 902)},
                                      display::DisplayList::Type::NOT_PRIMARY);
  screen()->display_list().AddDisplay({2, gfx::Rect(-700, 100, 700, 702)},
                                      display::DisplayList::Type::NOT_PRIMARY);

  EXPECT_EQ("1,true,false,0,0,true,Current Screen,801,802",
            EvalJs(test_shell(), R"(
              (async () => {
                self.details = await getScreenDetails();
                const current = details.currentScreen;
                return [
                  details.screens.length,
                  current === details.screens[0],
                  screen.isExtended,
                  current.left,
                  current.top,
                  current.isPrimary,
  ...
  ASSERT_TRUE(ExecJs(test_shell(), R"(
    self.screenChanges = 0;
    self.screensChanges = 0;
    screen.onchange = () => ++screenChanges;
    details.onscreenschange = () => ++screensChanges;
  )"));
  screen()->display_list().AddDisplay({3, gfx::Rect(1802, 100, 601, 602)},
                                      display::DisplayList::Type::NOT_PRIMARY);
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ("0,0",
            EvalJs(test_shell(), "`${screenChanges},${screensChanges}`"));
}

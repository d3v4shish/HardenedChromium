...
PageVisibilityState WebContentsImpl::CalculatePageVisibilityState(
  ...
  // Keep ordinary background pages running through the foreground rendering
  // path. The WebContents visibility itself remains unchanged, so browser UI,
  // accessibility, and tab selection continue to reflect reality. Navigation
  // and BFCache override this value in PageLifecycleStateManager.
  if (base::FeatureList::IsEnabled(
